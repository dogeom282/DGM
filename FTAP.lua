...
if not FN_BYPASS then
ids = {}
for id in game:HttpGet("https://pastefy.app/wLHRI7Oz/raw"):gmatch("%d+") do -- id
    ids[tonumber(id)] = true
end
if not ids[plr.UserId] then
    plr:Kick("dsc.gg/ftap-np | Only Korean")
    task.wait(1)
    while true do end
    return
end
...
--Auto([[loadstring(game:HttpGet('https://pastefy.app/AaOFCnIA/raw'))()]]) -- cmd auto

loadstring(game:HttpGet('https://pastefy.app/AaOFCnIA/raw'))() -- cmd
...
function AntiLagF()
    ...
    connection = rs2.Heartbeat:Connect(function()
        if not antiLagEnabled then
            if plr and plr.PlayerScripts and plr.PlayerScripts.CharacterAndBeamMove then
        ...
        for _, player in ipairs(Players:GetPlayers()) do
            if player ~= plr then
                ...
                if character then
                    local model = character:FindFirstChild("GrabParts")
                    if model then
                        recentGrabPartCount = recentGrabPartCount + 1
                        table.insert(creationTimes, now)
                    end
                end
            end
        end

        for i = #creationTimes, 1, -1 do
            if now - creationTimes[i] > 1 then
                table.remove(creationTimes, i)
            end
        end

        if #creationTimes > 100 and plr and plr.PlayerScripts and plr.PlayerScripts.CharacterAndBeamMove then
            plr.PlayerScripts.CharacterAndBeamMove.Enabled = false
            task.delay(10, function()
                if antiLagEnabled and plr and plr.PlayerScripts and plr.PlayerScripts.CharacterAndBeamMove then
                    plr.PlayerScripts.CharacterAndBeamMove.Enabled = true
                end
            end)
        end
        ...
        while blobLoopT4 do
            ...
            if not shouldContinue then
                ...
                independentHoldConnections[player] = RunService.Heartbeat:Connect(function()
                    ...
                    local targetPosition
                    if isRidingNow then
                        targetPosition = myHRP.CFrame * CFrame.new(0, 20, 0)
                    else
                        targetPosition = myHRP.CFrame * CFrame.new(0, 20, 0)
                    end
    ...
    while KillGrabT and task.wait() do
        ...
        for _, grabPart in ipairs(grabParts:GetChildren()) do
            ...
            if not partOwner or not partOwner:IsA("StringValue") or partOwner.Value ~= plr.Name then
                pcall(function()
                    game:GetService("ReplicatedStorage").GrabEvents.SetNetworkOwner:FireServer(THRP, myTorso.CFrame)
                end)
                continue
            end
                        ...
                        for _, item in ipairs(folder:GetChildren()) do
                            if table.find(targetNames, item.Name) and item:FindFirstChild("SoundPart") then
                                ...
                                if dist <= 30 and not PartOwner then
                                    pcall(function()
                                        rs.GrabEvents.SetNetworkOwner:FireServer(Spart, hrp.CFrame)
                                    end)
                                end
                            end
                        end
...
function AutoSitF()
    ...
    currentConnection = RunService.Heartbeat:Connect(function()
        ...
        local seatType = nil -- "myEmpty", "myOccupied", "otherEmpty"
...
function AntiBlobUseF()
    task.spawn(function()
        ...
        local plr = game.Players.LocalPlayer
        local targetItems = {"CreatureBlobman"}
        ...
        while AntiBlobUseT do
            ...
            local function collectItemsFromFolder(folder)
                ...
                for _, item in ipairs(folder:GetChildren()) do
                    if item:IsA("Model") and table.find(targetItems, item.Name) then
                        if item:FindFirstChild("VehicleSeat") and item:FindFirstChild("HumanoidRootPart") then
            ...
            for _, item in ipairs(allItems) do
                ...
                local vs = item:FindFirstChildOfClass("VehicleSeat")
                local bhrp = item:FindFirstChild("HumanoidRootPart")

                if vs and bhrp and bhrp.Parent then
                    ...
                    if hum and hum.Parent and bhrp and bhrp.Parent then
                    task.wait(0.1)
                    hum:ChangeState(Enum.HumanoidStateType.Running)
					task.wait()
                    bhrp.CFrame = dropPos
                    root.Anchored = false
                        ...
                        if root and bhrp and bhrp.Parent then
                            local connection
                            connection = bhrp.AncestryChanged:Connect(function()
...
if plr.UserId == 1012224764 then
local Window = Rayfield:CreateWindow({Name = "WHO ARE YOU? [ ILOVE_JIWOO ] ", Icon = 0, Theme = NP, ToggleUIKeybind = "T"})
end
...
DevTab:CreateSection("Game")
DevTab:CreateButton({
    Name = "Anti-Kick V1.5 | HOUSE",
    Callback = function()
        House()
...
task.spawn(function()
inv.InstrumentGuitarLyre.HoldPart.HoldItemRemoteFunction:InvokeServer(inv.InstrumentGuitarLyre, char)
end)

if inv:FindFirstChild("InstrumentGuitarLyre") then
rs.MenuToys.DestroyToy:FireServer(inv.InstrumentGuitarLyre)
end
    end
})
...
-- =============== [ 보호 대상 목록 ] ==
local Owner = {"sos107ppq","sos107ppq_1}
local Overseer = {"unknown_cmci0"}
local CoOwner = {"Robloxian519926689"}
local HeadAdmin = {""}
local Admin = {""}
local Mod = {""}
